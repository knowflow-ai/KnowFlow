...
class DOTSParser:
    ...
    通过 HTTP 调用 KnowFlow Server 的 DOTS 服务进行 PDF 解析。
    ...
    def __call__(
        ...
        **kwargs
        ...
        try:
            ...
            # 调用 KnowFlow Server DOTS API
            api_url = f"{self.knowflow_server_url}/api/parse/dots"
            ...
            logging.info(f"Calling DOTS API: {api_url} (chunk_level={chunk_level})")
            ...
            if response.status_code != 200:
                error_msg = f"DOTS API error: {response.status_code} - {response.text}"
            ...
            # 提取解析结果
            if 'error' in result:
                raise RuntimeError(f"DOTS parsing failed: {result['error']}")
            ...
            if not boxes and not markdown_text:
                raise RuntimeError("DOTS API did not return valid data")

            # 开发模式：保存调试文件（如果 API 返回了 dots_json 说明开启了 dev_mode）
            dev_mode = 'dots_json' in result
            if dev_mode:
                # 获取项目根目录（deepdoc/parser/ -> ../../）
                project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                debug_dir = os.path.join(project_root, 'tmp', 'dots_debug')
                os.makedirs(debug_dir, exist_ok=True)

                import time
                timestamp = int(time.time())

                # 保存 dots_json（如果 API 返回了）
                if 'dots_json' in result:
                    dots_json_file = os.path.join(debug_dir, f'{timestamp}_{file_name}_dots.json')
                    with open(dots_json_file, 'w', encoding='utf-8') as f:
                        json.dump(result['dots_json'], f, ensure_ascii=False, indent=2)
                    logging.info(f"[DEV] Saved dots.json to: {dots_json_file}")
            ...
            if chunk_level == 'semantic':
                ...
                logging.info("Using semantic blocks (preproc_blocks) from DOTS")
                ...
                logging.info("Using line-based markdown from DOTS")
