...
def chunk(filename, binary=None, from_page=0, to_page=100000,
    ...
    if not re.search(r"\.pdf$", filename, re.IGNORECASE):
        raise NotImplementedError("Regex chunking only supports PDF files")

    callback(0.1, "Start to parse.")
    logging.info("Using MinerU parser for regex chunking")
    ...
    callback(0.6, "Calling regex chunking service...")

    # 调用 KnowFlow Server 的正则分块服务
    try:
        ...
        callback(0.9, "Regex chunking completed.")

    except Exception as e:
        logging.error(f"Regex chunking service failed: {e}")
        callback(0.9, f"Regex chunking failed: {e}")
        raise
    ...
    logging.info(f"Regex chunking completed: {len(res)} chunks created")
